 ...
 *******************************************************************************/
package ac.soton.eventb.atomicitydecomposition.navigator.refiner;
 ...
 *
 */
public class FlowDiagramRefiner implements IRefinementParticipant {
	...
	private void refineExtensions(IEventBRoot concreteEventBRoot, IEventBRoot abstractEventBRoot, IProgressMonitor monitor) throws RodinDBException, CoreException {
		Machine machine = (Machine) EMFRodinDB.INSTANCE.loadElement(concreteEventBRoot);
		...
		try {
			machine.eResource().save(Collections.EMPTY_MAP);
	...
	private EventBElement refineEventBElement(EventBObject extension) {
		...
		if(concreteEventBElement instanceof FlowDiagram){
			TransactionalEditingDomain domain = TransactionUtil.getEditingDomain(extension);
			final FlowDiagram ext = (FlowDiagram)concreteEventBElement;
...
private static FlowDiagram setRefined(FlowDiagram fd) {
		fd.setCopy(true);
		
		for (Child child : fd.getRefine()) {

			
			if (child.eClass().getName() == "Leaf") {
				if(!((Leaf) child).getDecompose().isEmpty()) 
					for (FlowDiagram flow : ((Leaf) child).getDecompose()) {
						flow.setCopy(true);
						setRefined(flow);
					}
			}
			else if (child.eClass().getName() == "And") {
				for (Leaf leaf : ((And) child).getAndLink())
			...
			else if (child.eClass().getName() == "Or") {
				for (Leaf leaf : ((Or) child).getOrLink())
					if (!leaf.getDecompose().isEmpty())
						for (FlowDiagram flow : leaf.getDecompose()) {
							flow.setCopy(true);
							setRefined(flow);
			...
			else if (child.eClass().getName() == "Xor") {
				for (Leaf leaf : ((Xor) child).getXorLink())
					if (!leaf.getDecompose().isEmpty())
						for (FlowDiagram flow : leaf.getDecompose()) {
							flow.setCopy(true);
							setRefined(flow);
			...
			else if (child.eClass().getName() == "All") {
				if (!((All) child).getAllLink().getDecompose().isEmpty())
					for (FlowDiagram flow : ((All) child).getAllLink().getDecompose()) {
						flow.setCopy(true);
						setRefined(flow);
					}
			}
			else if (child.eClass().getName() == "One") {
				if (!((One) child).getOneLink().getDecompose().isEmpty())
					for (FlowDiagram flow : ((One) child).getOneLink().getDecompose()) {
						flow.setCopy(true);
						setRefined(flow);
					}
			}
	...
	protected String getExtensionID() {
		return AtomicitydecompositionPackage.FLOWDIAGRAMS_EXTENSION_ID;
	}
